...
x_train = MyGray2TorchRGB(MyResize(x_train)) / 255.0
x_test = MyGray2TorchRGB(MyResize(x_test)) / 255.0
...
net.load_state_dict(torch.load('model/poisoned_nontarget_pytorch.pth'))
...
for epoch in range(20):
    running_loss = 0.0
    for i, data in enumerate(trainloader_clean, 0):
...
torch.save(net.state_dict(), 'model/poisoned_nontarget_pytorch_fine.pth')
